...
import NumberAgency from './NumberAgency';
...
export default function AdminDashboard() {
  ...
  // Function to fetch total likes from the API
  const fetchTotalLikes = async () => {
    try {
      const response = await axios.get(`https://localhost:7125/api/Agency/agentrecords/${agentId1}`);
      const data = response.data;

      // Check if the data is a number (e.g., 1) and set it directly as the totalLikes
  ...
  const fetchBooking = async () => {
    try {
      const response = await axios.get(`https://localhost:7125/api/Bookings/ByAgency/${agentId}`);
      const data = response.data;

      // Check if the data is a number (e.g., 1) and set it directly as the numberofbooking
      if (typeof data === 'number') {
        setBooking(data);
      } else {
  ...
  const fetchFeedback = async () => {
    try {
      const response = await axios.get(`https://localhost:7125/api/FeedBacks/ByAgency/${agentId}`);
      const data = response.data;
  ...
  const handleLogout = () => {
    ...
    // Navigate to AgentLogin component
    navigate('/agentlogin');
  };
    ...
    <div className={`app ${darkMode ? 'dark-mode' : ''}`}>
      <nav>
        <div className="logo-name">
          ...
          <span className="logo_name">GOBIGO</span>
        </div>
        ...
        <div className="menu-items">
          <ul className="nav-links">
            ...
            <li>
                ...
                <Link className="nav-link link-name" to="/AgentAddPackage">
                      ImageGallery
                    </Link>
