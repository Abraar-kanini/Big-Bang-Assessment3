...
export default function AgentLogin() {
  ...
  const handleLogin = async () => {
    // Get the status and agent_id based on the agentName
    const statusUrl = `https://localhost:7125/api/AgentRegisters/Status/${agentName}`;
    try {
      const statusResponse = await axios.get(statusUrl);
      const { agent_id, status } = statusResponse.data;

      // Store the agent_id in local storage
      localStorage.setItem('agentId', agent_id);

      if (status === 'Approved') {
        // If status is Approved, proceed with the login
        ...
        try {
          const response = await axios.post(apiUrl, agentData);
        ...
        // Show error toast message for status not Approved
        toast.error('Login failed. Your account status is not approved.', {
          position: 'top-right',
          autoClose: 5000,
          hideProgressBar: true,
          closeOnClick: true,
      ...
      console.error('Error:', error.response);
      // Show error toast message for agentName not found
      toast.error('Agent not found. Please check your username.', {
        position: 'top-right',
        autoClose: 3000,
        hideProgressBar: true,
        closeOnClick: true,
    ...
    <div>
      ...
      <section>
        ...
        <div className="alternative-signup">
          <p>
            Not a member? <Link to="/AgentSignin" className="small text-muted">
                          Register here
                        </Link>
          </p>
        </div>
