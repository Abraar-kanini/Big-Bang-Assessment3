...
export default function BookingPage() {
  ...
  const encryptedUserId2 = localStorage.getItem('selectedAgencyId');
const agencystoredId = parseInt(AES.decrypt(encryptedUserId2, 'your_secret_key').toString(enc.Utf8), 10);
  // const userstoredId = localStorage.getItem('userId');
  const encryptedUserId1 = localStorage.getItem('userId');
const userstoredId = parseInt(AES.decrypt(encryptedUserId1, 'your_secret_key').toString(enc.Utf8), 10);
  ...
  const handlePayThePayment = () => {
    ...
    const tableData = [
      ['No of Person', paymentDetails.no_of_perons],
      ['No of Children', paymentDetails.no_of_childer],
      ['Rate per Day', rateForDay],
      ['Total Amount', paymentDetails.booking_amount],
      ['GST', "18%"],
      ['Total', paymentDetails?.booking_amount || '$85.00'],
    ];
